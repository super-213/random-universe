...
| `950–995` | 超远未来，从最后黑洞蒸发延伸至 `10^1200 年` |
| `995–1000` | 指数继续无界增长并渐近无限远时间；`1000` 表示 `T→∞` |

`10^308` 年以上的时间无法用 JavaScript 普通数值表示，因此超远未来内部保存 `log10(年)`，事件排序、标签和时间轴往返不需要构造溢出的年份。有限时间结局仍把“当前年龄—结局”整体重新映射到 `470–1000`，避免在大撕裂、大坍缩或真空衰变前跳过大段历史。`1×` 播放完整旅程约 35 分钟，早期宇宙和终局前会刻意放慢；`10×` 约 3 分 30 秒，`100×` 约 21 秒。
...
| 远未来 | 最后一颗普通恒星熄灭、白矮星碰撞、质子衰变时代、星系引力蒸发、流浪黑洞掠过、霍金末期爆发、黑矮星超新星、最后可探测信号 | 仅在渐近热寂分支生成，并按白矮星/黑洞宿主和远未来时间窗口裁剪；黑矮星超新星明确标记为高度推测 |
