...
- **条件宇宙结局**：宇宙可能走向渐近热寂、大/小撕裂、III 型有限尺度奇点、大坍缩或循环反弹，也可能被更早发生的真空衰变覆盖。
...
- **整个可观测宇宙**：从当前星系连续拉远到由 12,000 个代表性星系、超星系团、纤维与空洞构成的确定性宇宙网；种子会生成多孔网、弯曲星系墙、双叶结构、空洞壳层或扭转纤维带，纤维完全由星系密度呈现而不绘制连接线。
...
- **性能与无障碍**：进入探索器后分阶段构建星系，文明推演优先运行在 Web Worker；提供键盘选星、因果图文本描述和减少动态效果适配。
...
| 幽灵暗能量 | 12% | `w0 ∈ [-1.22,-1.035]`，`wa ∈ [0.015,0.14]` | 有限时间大撕裂 |
| 渐近幽灵能量 | 8% | `w→-1⁻`，其中 28% 为伪撕裂变体 | `T→∞` 小撕裂，无有限时间奇点 |
| III 型奇异流体 | 6% | `aₛ ∈ [2.2,14]` | 有限时间、有限尺度下密度与压力发散 |
| 反转势能 | 12% | `w0 ∈ [-0.98,-0.78]`，`wa ∈ [-0.08,0.08]` | 膨胀反转后大坍缩；其中 28% 启用循环反弹尾声 |

宇宙学常数、演化标量场、幽灵暗能量与反转势能的平滑背景采用有界的 Barboza–Alcaniz 型参数化：

```text
w(a) = w0 + wa(1 - a) / [a² + (1 - a)²]
```

小撕裂分支在 `a > 1` 后改用渐近式，以保证 `w→-1⁻`：
...
`p = 1` 让暗能量密度无界增长，但把膨胀率发散推到无限时间；`p = 2` 的积分有界，对应有限的渐近撕裂强度。
...
当 `E² ≤ 0` 时膨胀停止，模拟器继续反向积分到 `a < 10^-8`，得到大坍缩时间。幽灵暗能量用有限剩余寿命近似推演大撕裂；渐近幽灵能量则使束缚结构在有限时间解体，但把膨胀率发散推迟到 `T→∞`。伪撕裂使膨胀惯性力渐近有限上限，因此只移除部分弱束缚结构。

III 型奇点使尺度因子趋近种子化的有限值，同时让密度、压力与高能辐射在有限时间内发散。循环反弹不是独立的经典结局，而是大坍缩进入高曲率区后的高度推测尾声；它会在时间轴最后 0.8% 中呈现反弹脉冲与新一轮膨胀。
...
│  └─ 结构效率、恒星规模与恒星寿命
├─ 暗能量与真空状态
│  └─ 热寂 / 大撕裂 / 小撕裂 / III 型奇点 / 大坍缩—反弹 / 真空衰变
├─ 星系形态与中央黑洞
│  └─ 恒星轨道、活动星系核与可发生事件
├─ 天体事件日程
│  └─ 残骸、引力场、辐射束、文明损伤与恢复
...
- **六类条件暗能量模型与多种最终结局**。伪撕裂属于小撕裂的模型变体，循环反弹属于大坍缩的尾声；真空衰变可能抢在其他结局之前发生。
...
局部星系群由当前种子独立生成 4–6 个伴星系。跨星系舰队具有速度、人口、补给、距离、预计抵达时间与确定性结局；伴星系和航线仍保留完整模拟。选择“查看整个宇宙”后，当前星系与局部群会缩小为宇宙网中一个偏离中心的已标记节点，外围展示 12,000 个代表性星系以及由点云密度自然形成的超星系团、纤维和空洞，不绘制宇宙网连接线。星系会沿由附近超星系团和大尺度形态决定的连续流场缓慢运动，当前星系标记和星系际航行会跟随同一轨迹。全宇宙文明层会额外推演其他星系之间的启航、在途、抵达、外域建立和失联事件，并以移动光点与短促脉冲呈现。
...
灵感来源包括《基地》的文明衰落与知识保存、《黑暗森林》的暴露与静默、《时间之子》的提升物种、《环形世界》的恒星尺度巨构、《七夏娃》的避难与文明重建、《火星三部曲》的行星改造、《雪崩》的信息危害、《太空漫游》的先驱遗迹、《永恒战争》的相对论文化分化、《恒星制造者》的恒星尺度运动、《太阳之屋》的深时巡游与记忆汇合、《离散》的数字心智与宇宙逃逸，以及《希尔德的阶梯》对人造真空相变边界的警示。实现只提取通用母题，不复用作品角色、组织或专有情节。
...
| 致密天体与透镜观测 | 快速射电暴、X 射线双星爆发、黑洞吸积态转换、脉冲星停止/恢复、致密残骸与流浪行星微透镜、黑洞光子环耀斑 | 只从已经形成的兼容源中选择位置；流浪行星候选保留质量—距离简并和极宽轨道替代解释 |
| 行星与生命 | 星际天体掠过、生物大氧化、大型小行星撞击、岩石胚种转移、失控温室、全球冰封/解冻、生物信号消失 | 使用“撞击 → 气候跃迁 / 岩石转移 → 生物信号变化”因果链；胚种会在邻近恒星域判定落地成败，成功节点将提高后续殖民和生物圈承载力 |
| 宇宙学候选 | 宇宙弦透镜候选 | 用沿窄带出现的无畸变成对像表达锥形时空透镜；界面单独标为“宇宙学候选”，并保留普通星系对齐和仪器误差解释 |
| 文明观测 | 异常凌日、红外废热、窄带漂移、激光信标、文明信号静默、巨型工程遮蔽、相对论舰队尾迹、恒星自行异常 | 依赖真实行星系统、稳定恒星工程、已知信号或跨星系舰队前置事件；恒星推进会持久改变母星位置、光行时、能耗和物流效率 |
| 深时文明 | 深时巡游档案汇合、低温计算夏眠唤醒 | 档案汇合会改变记忆完整度、凝聚力、治理与科研；数字文明会先进入低功耗夏眠，获得深时存续加成，再于冷背景阶段恢复计算、通信与扩张 |
...
- 动态暗能量、大/小/伪撕裂、III 型奇点、反转势能、循环反弹和真空衰变都是条件模型，生成权重不是观测概率。
...
- [Frampton 等：小撕裂](https://arxiv.org/abs/1106.4996)
- [Frampton 等：伪撕裂](https://arxiv.org/abs/1112.2964)
- [Nojiri、Odintsov 与 Tsujikawa：有限时间未来奇点分类](https://arxiv.org/abs/hep-th/0501025)
- [Steinhardt 与 Turok：循环宇宙](https://journals.aps.org/prd/abstract/10.1103/PhysRevD.65.126003)
- [NASA Roman：流浪行星与微透镜](https://www.nasa.gov/missions/roman-space-telescope/new-study-reveals-nasas-roman-could-find-400-earth-mass-rogue-planets/)
- [NASA：星际天体 ʻOumuamua](https://science.nasa.gov/solar-system/comets/oumuamua/)
- [Sazhin 等：宇宙弦的引力透镜特征](https://arxiv.org/abs/astro-ph/0611744)
- [Adams 与 Napier：行星系之间的岩石转移](https://arxiv.org/abs/2205.07799)
- [Forgan：恒星推进器的凌日技术迹象](https://arxiv.org/abs/1306.1672)
- [Sandberg 等：低温计算夏眠假说](https://arxiv.org/abs/1705.03394)
...
- Node.js Test Runner：仿真、时间映射、加载取消、模块边界、视觉注册表和时间轴筛选测试

## 实现与加载流程

`src/main.js` 是轻量入口：载入样式后创建 `createExplorerApp()`。应用实例持有当前宇宙、场景资源、文明快照和时间轴状态；页面卸载或热更新时会清理资源。创世界面先展示基础场景，并在浏览器空闲时通过 `src/explorer-dependencies.js` 预加载探索器依赖和星系预览。进入星系后的完整构建由 `src/features/galaxy/galaxy-hydrator.js` 分阶段安排，并在种子或页面状态变化时取消过期任务。文明历史优先交给 Web Worker 计算，Worker 不可用时回退到主线程。

代码按职责分层：`domain/` 保存种子化数据和时间计算，`simulation/` 推进事件与文明，`rendering/` 管理 Three.js 场景和视觉生命周期，`ui/` 渲染 DOM；`app/` 管理会话、帧循环和浏览器调度，`features/` 编排星系加载与时间轴交互。事件视觉的创建、更新和动画通过注册表分派。样式由 `src/style.css` 按顺序引入 `src/styles/` 中的页面、时间轴、浮层与响应式规则。
...
├── src/
│   ├── main.js                    # 入口：创建和销毁探索器应用实例
│   ├── explorer-dependencies.js   # 进入宇宙后按需加载的探索器依赖
│   ├── app/
│   │   ├── explorer-app.js        # 应用组合、场景状态和交互接线
│   │   ├── explorer-session.js    # 每个应用实例的会话状态
│   │   ├── frame-loop.js          # 渲染帧循环
│   │   ├── browser-scheduler.js   # 浏览器空闲调度与主线程让步
│   │   └── workers/              # 文明 Worker 入口、调度与主线程降级
│   ├── features/
│   │   ├── galaxy/galaxy-hydrator.js     # 星系分阶段构建与过期任务取消
│   │   └── timeline/timeline-controller.js # 时间轴播放、拖动与筛选交互
│   ├── domain/
│   │   ├── random.js             # 种子规范化、哈希与伪随机数
│   │   ├── math.js               # 不依赖 Three.js 的向量与数学工具
│   │   ├── universe.js           # 宇宙参数、派生量与早期里程碑
│   │   ├── cosmic-fate.js        # 暗能量积分与条件宇宙结局
│   │   ├── cosmic-time.js        # 物理时间和可视时间轴映射
│   │   ├── cosmic-web.js         # 可观测宇宙的多形态结构、星系采样与确定性流场
│   │   ├── stellar-dawn.js       # 第一代恒星的空间点火与形成
│   │   ├── stellar-population.js # 恒星样本、光谱与寿命
...
│   │   ├── civilization-lifecycle.js # 文明存续与衰亡规则
...
│   │   ├── rare-events.js        # 环境、观测与远未来条件事件
│   │   ├── event-occurrence.js   # 泊松重复事件调度
│   │   ├── black-hole-gravity.js # 星系势与双黑洞近场响应
│   │   └── compact-objects.js    # 致密残骸与蒸发时间
│   ├── rendering/                 # Three.js 场景、资源、视觉注册表与专门系统
│   ├── ui/                        # 宇宙数据、时间轴筛选、文明编年史与因果图
│   ├── assets/                    # 构建时打包资源
│   ├── styles/                    # 页面、时间轴、浮层和响应式样式
│   └── style.css                  # CSS 导入入口
├── test/                          # 仿真、架构、加载、视觉注册表及 UI 测试
├── scripts/subset-font.mjs        # 从页面源码重新生成得意黑字符子集
├── public/fonts/                  # 字体与许可证
├── wrangler.jsonc                 # Cloudflare 静态资源部署配置（使用 dist/）
